-- Apply Comprehensive Documents & E-Signature System Migration
-- Run this script in Supabase Studio SQL Editor

-- First, check if tables already exist and drop them safely
DO $$ 
BEGIN
    -- Drop dependent tables first
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'warranty_claim_updates') THEN
        DROP TABLE public.warranty_claim_updates CASCADE;
        RAISE NOTICE 'Dropped warranty_claim_updates table';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'warranty_claims') THEN
        DROP TABLE public.warranty_claims CASCADE;
        RAISE NOTICE 'Dropped warranty_claims table';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'document_versions') THEN
        DROP TABLE public.document_versions CASCADE;
        RAISE NOTICE 'Dropped document_versions table';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'document_signatures') THEN
        DROP TABLE public.document_signatures CASCADE;
        RAISE NOTICE 'Dropped document_signatures table';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'document_templates') THEN
        ...
        RAISE NOTICE 'Dropped document_templates table';
    END IF;

    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'documents') THEN
        DROP TABLE public.documents CASCADE;
        RAISE NOTICE 'Dropped documents table';
    END IF;
END $$;
...
CREATE TABLE public.warranty_claim_updates (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
M REATE OR REPLACE FUNCTION generate_document_number(doc_type TEXT, p_org_id TEXT DEFAULT 'temp-org')
RETURNS TEXT AS $$
DECLARE
    prefix TEXT;
    counter INTEGER;
...
CREATE INDEX idx_document_signatures_signer_email ON public.document_signatures(signer_email);
CREATE INDEX idx_document_signatures_status ON public.document_signatures(status);

CREATE INDEX idx_warranty_claims_org_id ON public.warranty_claims(org_id);
CREATE INDEX idx_warranty_claims_status ON public.warranty_claims(status);
CREATE INDEX idx_warranty_claims_customer_email ON public.warranty_claims(customer_email);
CREATE INDEX idx_warranty_claims_created_at ON public.warranty_claims(created_at DESC);
...
SELECT 
    'Comprehensive Documents & E-Signature System installed successfully!' as status,
    COUNT(*) as sample_documents
FROM public.documents;

SELECT 
    'Sample warranty claims created!' as status,
    COUNT(*) as sample_claims
FROM public.warranty_claims;
