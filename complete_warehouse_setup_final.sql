-- Complete Warehouse Setup Script - Final Version
-- This script creates all necessary tables and populates them with data in the correct order
...
-- Step 2: Create warehouse_locations table SECOND
...
-- Step 3: Create warehouse_stock table THIRD (depends on products and locations)
...
-- Step 4: Create warehouse_movements table FOURTH (depends on products and locations)
...
-- Step 10: Insert warehouse products FIRST (has unique constraint on sku)
...
-- Step 11: Insert warehouse locations SECOND (has unique constraint on code)
...
-- Step 12: Insert stock items for the products THIRD (no unique constraint, so no ON CONFLICT)
...
-- Step 13: Insert sample movements FOURTH (no unique constraint, so no ON CONFLICT)
