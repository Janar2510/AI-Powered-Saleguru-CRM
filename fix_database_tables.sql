...
-- Update existing warehouse location types to match new constraint
...
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'warehouse_stock' AND column_name = 'min_quantity') THEN
        ALTER TABLE warehouse_stock RENAME COLUMN min_quantity TO min_stock_level;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'warehouse_stock' AND column_name = 'max_quantity') THEN
...
-- Create accounting_rules table if it doesn't exist
