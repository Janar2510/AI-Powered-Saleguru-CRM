...
-- Step 4: Create RLS policies for warehouse_products (only if they don't exist)
DO $$
BEGIN
...
-- Step 5: Create RLS policies for accounting_rules (only if they don't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'accounting_rules' AND policyname = 'Users can view all rules') THEN
        CREATE POLICY "Users can view all rules" ON accounting_rules FOR SELECT USING (true);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'accounting_rules' AND policyname = 'Users can insert rules') THEN
        CREATE POLICY "Users can insert rules" ON accounting_rules FOR INSERT WITH CHECK (auth.role() = 'authenticated');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'accounting_rules' AND policyname = 'Users can update rules') THEN
        CREATE POLICY "Users can update rules" ON accounting_rules FOR UPDATE USING (auth.role() = 'authenticated');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'accounting_rules' AND policyname = 'Users can delete rules') THEN
        CREATE POLICY "Users can delete rules" ON accounting_rules FOR DELETE USING (auth.role() = 'authenticated');
    END IF;
END $$;

-- Step 6: Insert warehouse products (only if they don't exist)
...
-- Step 7: Insert accounting rules (only if they don't exist)
...
-- Step 8: Insert stock items for the products (only if they don't exist)
...
-- Step 9: Insert some sample movements (only if they don't exist)
