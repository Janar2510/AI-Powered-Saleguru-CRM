-- Ultra Safe Deals Migration Script
-- Apply this in Supabase Studio SQL Editor
-- This version completely avoids problematic UUID operations
...
-- SIMPLE stage updates - avoid any UUID operations
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'deals' AND column_name = 'stage') THEN
        -- Simply update any NULL or empty stage values to 'new'
        UPDATE public.deals 
        SET stage = 'new'
        WHERE stage IS NULL OR stage = '';
        
        RAISE NOTICE 'Updated stage values for deals with NULL or empty stages';
    END IF;
END $$;

-- Clean up problematic stage_id column if it exists and causes issues
DO $$
BEGIN
    -- If stage_id exists and is causing problems, we can remove it or ignore it
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'deals' AND column_name = 'stage_id') THEN
        -- Just leave stage_id as is - don't try to migrate its values
        -- We'll rely on the stage column going forward
        RAISE NOTICE 'stage_id column exists but will not be migrated to avoid UUID conflicts';
    END IF;
END $$;
...
BEGIN
    ...
    IF deal_count = 0 THEN
        ...
        RAISE NOTICE 'Inserted % sample deals', 8;
    ELSE
        RAISE NOTICE 'Table already contains % deals, skipping sample data', deal_count;
    END IF;
END $$;
...
DECLARE
    ...
    total_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO total_count FROM public.deals;
    SELECT COUNT(*) INTO stage_count FROM public.deals WHERE stage IS NOT NULL AND stage != '';
    
    RAISE NOTICE 'Migration completed successfully!';
    RAISE NOTICE 'Total deals: %', total_count;
    RAISE NOTICE 'Deals with stage values: %', stage_count;
END $$;
