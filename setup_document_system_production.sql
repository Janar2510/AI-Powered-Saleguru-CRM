-- Document Builder System Setup for SaleToru CRM - PRODUCTION READY VERSION

-- Issue: Creating extensions in a DO block with exception handling is not ideal
-- Extensions should be created in the extensions schema
...
-- Add function to handle upsert for branding
CREATE OR REPLACE FUNCTION upsert_branding()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO branding (user_id, company_name)
    VALUES (NEW.id, 'SaleToru CRM')
    ON CONFLICT (user_id) DO NOTHING;
    RETURN NEW;
END;
$$ language 'plpgsql' SECURITY DEFINER SET search_path = '';

-- Create trigger to automatically create branding entry for new users
CREATE TRIGGER create_branding_for_new_user
AFTER INSERT ON auth.users
FOR EACH ROW EXECUTE FUNCTION upsert_branding();

-- Insert default templates (with NULL user_id for system defaults)
-- Issue: ON CONFLICT clause in template insertion is now correct with unique constraint
