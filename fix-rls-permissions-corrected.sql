-- Fix RLS Permissions for Deals Table (Corrected)
...
-- Test the permissions by selecting data
SELECT 
...
-- Test update permissions (corrected syntax)
UPDATE public.deals 
SET description = 'Updated via RLS test - ' || NOW()::text
WHERE org_id = 'temp-org' 
AND id = (SELECT id FROM public.deals WHERE org_id = 'temp-org' ORDER BY created_at LIMIT 1);
